# After the Bad One

How do you let a bad shot go — actually go, not just pretend it is gone?

[BEAT]

First, settle for a minute — wherever this finds you, walking, warming up, or
somewhere quiet. If your eyes can close, let them. If you are moving, keep them open
and soft.

In, through your nose.

[PAUSE]

And out, through your mouth, longer than you took it in.

[PAUSE]

Again. In.

[PAUSE]

And out, longer.

[PAUSE — LONG]

[BEAT]

This is not about avoiding the bad shot — you cannot. It is about the ninety seconds
after it, because that is where its real cost is decided.

Golf punishes carrying. The game gives you a bad shot and then hands you five
minutes to walk with it, and an unprocessed emotion does not sit quietly for five
minutes. Suppressed — jaw set, told to move on before it has been looked at — it
does not leave. It rebounds. It colors the next decision toward fear or revenge, it
makes you rush the routine or abandon it, and one bad swing quietly becomes three
bad holes. The strokes you lose to a bad shot are rarely the shot. They are the
carry.

And listen to the sentence you say after a bad one, because your system treats your
own words as coaching input. There are two kinds. I always block it under pressure —
that is a verdict. Absolute, permanent, aimed at you, and it gives you nothing to do
next except be a worse golfer. That came out left — that is a description. Specific,
temporary, aimed at the ball, and it keeps the one thing the shot actually gave you,
which is information. Same swing, two different futures, depending on which sentence
follows it.

[BEAT]

The tool is a post-shot routine. You have a routine at the front of the shot; almost
nobody builds the one at the back. Three steps, and all three fit inside the walk to
the ball.

Step one: look at it. Watch the ball until it finishes. Turning away feels like
composure, but it is avoidance — it throws away the shot's information and keeps its
emotion. Exactly backwards.

Step two: one sentence, and it describes. That started left of the line. That was
short. Never a verdict, never about you. Catch a verdict forming and reframe it —
neutral, specific, forward-looking.

Step three: a physical release. One repeatable act that marks the end of the event.
Re-grip the club and set it back in the bag. Pull the glove tab. One long exhale on
the third step of the walk. Pick one and use the same one every time. The reason it
is physical: the body closes files the mind will argue about. An action is an
ending. A thought is a negotiation.

Two more pieces, because they decide the card. The damage from a bad hole is the
strokes on that hole — never the strokes plus the next three. That second part is
optional, and it is the only part you control. And after two in a row, the unit
shrinks. Not the round, not the nine — this swing. You are not saving anything. You
are hitting one shot.

Last, the honest part. The reset is not a mood. You will run all three steps and
still feel the heat — normal, and fine. The routine is not there to make you feel
good. It is there to keep the next decision clean while the feeling drains, and the
feeling drains faster while it runs.

[BEAT — HOLD]

Now rehearse it, on a real one. Bring back a shot from a recent round that actually
cost you — the one you carried. Put yourself there, just after impact.

[PAUSE — LONG]

Watch it finish. All the way to stopping. Now the sentence — describe it, nothing
else.

[PAUSE — LONG]

Now the release. Your act, the one you have chosen. Do it, start the walk, and
notice the file close behind you.

[PAUSE — LONG]

And the two-in-a-row version. Another one goes wrong, right after. Feel the pull to
fix the round in one swing. Shrink the unit: this swing. Run the three steps, and
walk.

[PAUSE — LONG]

[BEAT — HOLD]

The one thing: the shot costs what it costs. Everything after that, you are choosing
to pay.
